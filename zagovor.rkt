...
[define (mmap-macro f e)
  [cond [(empty? e) e]
        [#t (:: (call f (list (::-e1 e))) (mmap-macro f (::-e2 e)))]]]

; main fucntion
(define (mi izraz okolje)
  (letrec ([mii (lambda (e env)
                  (cond
                    [(mmap? e) (let ([lst (mii (mmap-e e) env)])
                               (cond [(empty? lst) lst]
                                     [(::? lst) (let ([res (mii (mmap (mmap-f e) (::-e2 lst)) env)])
                                                  (:: (mii (call (mmap-f e) (list (::-e1 lst))) env) res))]))]
                    ...
                    [(square? e) (let ([val_e (mii (square-e1 e) env)])
                                   (cond
                                     [(int? val_e) (int (expt (int-n val_e) 2))]
                                     [#t (error "square err")]))]
...
;(mi (square (add (int 1) (int 2)))) null)
;(mi (square (int 10)) null)
